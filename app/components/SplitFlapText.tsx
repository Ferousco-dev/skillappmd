...
export default function SplitFlapText({
  ...
  const phrasesKey =
    typeof text === 'string' ? text : sourceWords.map(word => String(word ?? '')).join(SEPARATOR)
  const phrases = useMemo(() => phrasesKey.split(SEPARATOR), [phrasesKey])
  ...
  useEffect(() => {
    ...
    const animateTo = (targetPhrase: string) => {
      ...
      const plans: Plan[] = targetChars
        .map((targetChar, index): Plan | null => {
      ...
      {tiles.map((tile, index) => (
        <span className="split-flap-text__tile" aria-hidden="true" key={`${index}-${tiles.length}`}>
          <span className="split-flap-text__half split-flap-text__half--top">
            <span className="split-flap-text__char">{tile.current === ' ' ? NBSP : tile.current}</span>
          </span>
          ...
          {tile.flipping && (
            <>
              ...
              >
                <span className="split-flap-text__char">{tile.next === ' ' ? NBSP : tile.next}</span>
              </span>
            </>
          )}
        </span>
