...
test('TC-012 REQ-042 content hash is stable and byte-sensitive', async () => {
  assert.equal(contentHash('hello'), contentHash('hello'));
  assert.notEqual(contentHash('hello'), contentHash('hello '));
});

test('TC-013 REQ-043 normalisation collapses CRLF/LF and trailing whitespace', async () => {
  assert.equal(normalisedHash('a\r\nb\r\n'), normalisedHash('a\nb\n'));
  assert.equal(normalisedHash('a   \nb\t\n'), normalisedHash('a\nb\n'));
  assert.equal(normalisedHash('a\nb'), normalisedHash('a\nb\n\n\n'));
});

test('TC-014 REQ-042/043 normalisation does NOT collapse genuinely different content', async () => {
  assert.notEqual(normalisedHash('a\nb\n'), normalisedHash('a\nc\n'));
});

test('TC-015 normaliseText strips BOM and enforces exactly one final newline', async () => {
...
test('TC-017 NFR-033 partition key derives from the hash without schema change', async () => {
  assert.equal(partitionKey(contentHash('x')).length, 2);
});

test('TC-018 DOM-001 occurrence key requires all four components', async () => {
...
test('TC-019 REQ-016 idempotency key is deterministic per stage', async () => {
...
test('TC-020 REQ-044 identical content resolves to one canonical skill', async () => {
...
test('TC-021 REQ-043 CRLF variant resolves as NEAR_DUPLICATE, not a new skill', async () => {
  const body = 'name: x\nbody';
...
test('TC-022 REQ-045 same name, different content is NOT a duplicate', async () => {
  const r = resolveRelationship(
...
test('TC-023 REQ-048 fork metadata is a signal distinct from content equality', async () => {
