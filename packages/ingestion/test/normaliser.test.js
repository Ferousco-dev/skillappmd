...
test('TC-111 REQ-039 normalisation produces a record with mandatory attribution', () => {
  const c = build();
...
test('TC-112 NFR-004 a discovery record without a URL is REJECTED at normalisation', () => {
  assert.throws(() => build({ url: '' }), /NFR-004/);
  assert.throws(() => build({ repo_full_name: '' }), /NFR-004/);
});

test('TC-113 DOM-006/REQ-040 every field origin is classified as fact or inference', () => {
  const c = build();
...
test('TC-114 DEC-006 L2 repository licence and L3 frontmatter claim are recorded separately', () => {
  const c = build({}, 'MIT');
...
test('TC-115 DEC-018 no repository licence yields the explicit unknown state', () => {
  const c = build({}, null);
...
test('TC-116 REQ-060 an L2/L3 conflict is flagged and the more restrictive applied', () => {
  const raw = '---\nname: s\ndescription: d\nlicense: MIT\n---\nb';
...
test('TC-117 REQ-036 unknown frontmatter keys survive into the canonical record', () => {
  const raw = '---\nname: s\ndescription: d\nx-custom: preserved\n---\nb';
  const c = normalise({ discovery: discovery(), parsed: parseSkill(raw), rawText: raw,
...
test('TC-118 DOM-006 the inferred compartment exists and is empty in Phase 1', () => {
  const c = build();
  assert.deepEqual(c.inferred, {}, 'shape exists so AI phases need no schema change');
});

test('TC-119 REQ-039 a parse failure still yields an attributable record', () => {
...
test('TC-120 NFR-038 normalisation refuses an implicit clock', () => {
  assert.throws(() => normalise({ discovery: discovery(), parsed: parseSkill(RAW), rawText: RAW }),
    /UTC timestamp/);
});

test('TC-121 NFR-001 normalisation is deterministic for identical input', () => {
