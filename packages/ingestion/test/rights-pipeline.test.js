...
const rig = () => { const s = new SqliteCanonicalStore(':memory:'); s.migrate({ now: NOW }); return s; };
...
function ingest(store, repo, raw, repoLicence) {
  const d = disco(repo);
  const canonical = normalise({ discovery: d, parsed: parseSkill(raw), rawText: raw, repoLicence, now: NOW });
  resolveOccurrence({ store, discovery: d, canonical, fingerprints: fingerprint(raw), now: NOW });
  return canonical;
}
const skill = (n, lic = null) =>
  `---\nname: ${n}\ndescription: d\n${lic ? `license: ${lic}\n` : ''}---\nBody ${n}.`;

test('TC-138 DEC-009 the corpus reality: no repository licence means UNKNOWN, not permitted', () => {
  ...
  const s = rig();
  const c = ingest(s, 'a/no-licence', skill('a'), null);
...
test('TC-139 REQ-057 NOASSERTION is UNKNOWN, never a guess', () => {
  // GitHub emits NOASSERTION when a licence file exists but is unrecognised.
  // It appeared 4 times in a 131-record real sample.
  const s = rig();
  const c = ingest(s, 'a/noassert', skill('a'), 'NOASSERTION');
  assert.equal(c.licence.l2_repository.spdx, UNKNOWN);
  assert.equal(c.rights.state, RIGHTS_STATE.UNKNOWN);
  s.close();
});

test('TC-140 NFR-006 the store holds no redistributable record lacking L2 evidence', () => {
  const s = rig();
  ingest(s, 'a/mit', skill('a'), 'MIT');
  ingest(s, 'b/none', skill('b'), null);
  ingest(s, 'c/gpl', skill('c'), 'GPL-3.0');
  ingest(s, 'd/claim', skill('d', 'MIT'), null);        // L3 claim, no L2
  const rows = s.cursorScan({ limit: 100 }).rows;
...
test('TC-141 DEC-018 an L3 claim without L2 backing yields UNKNOWN, not the claimed licence', () => {
  const s = rig();
  const c = ingest(s, 'a/claim', skill('a', 'MIT'), null);
...
test('TC-142 REQ-060 a real L2/L3 conflict is flagged and resolved to the more restrictive', () => {
  const s = rig();
  const c = ingest(s, 'a/conflict', skill('a', 'MIT'), 'AGPL-3.0');
  assert.equal(c.licence.conflict, true);
  assert.equal(c.licence.effective, 'AGPL-3.0');
  assert.equal(c.rights.redistributable, false);
  assert.equal(JSON.parse(s.cursorScan().rows[0].licence_json).l3_declared.spdx, 'MIT',
    'both layers survive into storage');
  s.close();
});

test('TC-143 DEC-019 retention follows rights, and unknown gets the shortest', () => {
  const s = rig();
  assert.equal(ingest(s, 'a/none', skill('a'), null).retentionPolicy, 'process-then-delete');
  assert.equal(ingest(s, 'b/gpl', skill('b'), 'GPL-3.0').retentionPolicy, 'short');
  assert.equal(ingest(s, 'c/mit', skill('c'), 'MIT').retentionPolicy, 'standard');
  s.close();
});

test('TC-144 NFR-004/NFR-025 attribution survives the whole pipeline into storage', () => {
  const s = rig();
  ingest(s, 'owner/repo', skill('a'), 'MIT');
  const r = s.cursorScan().rows[0];
...
test('TC-145 NFR-005 every stored field origin is classifiable as fact or inference', () => {
  const s = rig();
  ingest(s, 'a/one', skill('a'), 'MIT');
  const fo = JSON.parse(s.cursorScan().rows[0].provenance_json).field_origins;
