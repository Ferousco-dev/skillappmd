...
function rig() { const s = new SqliteCanonicalStore(':memory:'); s.migrate({ now: NOW }); return s; }
...
function ingest(store, repo, raw, path = 'SKILL.md', extra = {}) {
  const d = disco(repo, path, extra);
  const parsed = parseSkill(raw);
  const canonical = normalise({ discovery: d, parsed, rawText: raw, repoLicence: 'MIT', now: NOW });
  return resolveOccurrence({ store, discovery: d, canonical, fingerprints: fingerprint(raw), now: NOW });
}

test('TC-122 REQ-042/REQ-043 fingerprints are computed at both tiers', () => {
...
test('TC-123 REQ-044 byte-identical skills in different repos collapse to ONE canonical', () => {
  const s = rig();
  const a = ingest(s, 'alice/repo', SKILL);
  const b = ingest(s, 'bob/fork', SKILL);
...
test('TC-124 REQ-046 dedup collapses identity, NOT evidence', () => {
  const s = rig();
  const repos = ['a/one', 'b/two', 'c/three', 'd/four'];
  let id;
  for (const r of repos) id = ingest(s, r, SKILL).canonicalId;
  assert.equal(s.counts().canonical, 1);
  assert.equal(s.counts().occurrences, 4);
  const occ = s.listOccurrences({ canonicalId: id }).rows;
...
test('TC-125 REQ-043 a CRLF variant is a NEAR_DUPLICATE, not a second skill', () => {
  const s = rig();
  const a = ingest(s, 'a/one', SKILL);
  const b = ingest(s, 'b/two', SKILL.replace(/\n/g, '\r\n'));
  assert.equal(b.canonicalId, a.canonicalId);
  assert.equal(b.relationship, RELATIONSHIP.NEAR_DUPLICATE);
  assert.match(b.reason, /normalisation/);
  assert.equal(s.counts().canonical, 1, 'line endings must not inflate the canonical count');
  s.close();
});

test('TC-126 REQ-045 same name, different content are DIFFERENT skills', () => {
  const s = rig();
  const one = '---\nname: pdf\ndescription: Extract text from PDFs.\n---\nA';
  const two = '---\nname: pdf\ndescription: Fill PDF forms.\n---\nB';
  const a = ingest(s, 'a/one', one);
  const b = ingest(s, 'b/two', two);
  assert.notEqual(b.canonicalId, a.canonicalId);
  assert.equal(s.counts().canonical, 2, 'name equality is never evidence of duplication');
  s.close();
});

test('TC-127 REQ-016/NFR-009 re-ingesting the same occurrence 10 times changes nothing', () => {
  const s = rig();
  for (let i = 0; i < 10; i++) ingest(s, 'a/one', SKILL);
  assert.equal(s.counts().canonical, 1);
  assert.equal(s.counts().occurrences, 1);
  s.close();
});

test('TC-128 REQ-046 the same content at two PATHS in one repo is two occurrences', () => {
  const s = rig();
  const a = ingest(s, 'a/one', SKILL, 'skills/x/SKILL.md');
  const b = ingest(s, 'a/one', SKILL, 'skills/y/SKILL.md');
  assert.equal(b.canonicalId, a.canonicalId);
  assert.equal(s.counts().occurrences, 2, 'path is part of occurrence identity (DOM-001)');
  s.close();
});

test('TC-129 REQ-048 fork metadata is recorded as lineage, separate from content equality', () => {
  const s = rig();
  const r = ingest(s, 'b/fork', SKILL, 'SKILL.md', { is_fork: 1 });
...
test('TC-130 DEC-012 the git blob sha is reproduced exactly for cross-checking', () => {
...
test('TC-131 REQ-047 grouping comparison agrees with itself on synthetic groups', () => {
  const rows = [
...
test('TC-132 REQ-047 a grouping disagreement is REPORTED, never silently reconciled', () => {
...
test('TC-133 NFR-038 dedup resolution refuses an implicit clock', () => {
  const s = rig();
  assert.throws(() => resolveOccurrence({ store: s, discovery: disco('a/b'),
    canonical: {}, fingerprints: fingerprint(SKILL) }), /UTC timestamp/);
  s.close();
});

test('TC-134 DEF-003 a non-scalar name or description does not reach a column', () => {
  ...
  const s = rig();
  const raw = '---\nname: alpha\ndescription:\n  nested:\n    deep: x\n---\nBody.';
  assert.doesNotThrow(() => ingest(s, 'a/one', raw));
  const row = s.cursorScan().rows[0];
...
test('TC-135 DEC-031 the store names the offending field rather than emitting a bind error', () => {
  const s = rig();
  const raw = '---\nname: alpha\ndescription: d\n---\nB';
  const d = disco('a/one');
  ...
  assert.throws(() => s.upsertCanonical(canonical), /cannot store field "declared\.name".*object/);
  s.close();
});
