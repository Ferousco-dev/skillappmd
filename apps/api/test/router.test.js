...
async function rig({ skills = 3, limiter = null } = {}) {
...
const GET = async (router, path, query = {}, clientId = 'test') =>
  await router.handle({ method: 'GET', path, query, clientId });

test('TC-146 REQ-064 the API is versioned and reports health', async () => {
  const { router, store } = await rig();
  const r = await GET(router, '/api/v1/health');
  assert.equal(r.status, 200);
  assert.equal(r.body.schema_version, SCHEMA_VERSION, 'health reports the live schema version');
  store.close();
});

test('TC-147 REQ-065 a skill is returned with provenance, rights and attribution', async () => {
...
test('TC-148 REQ-070/DOM-006 facts and inferences are separate objects on the wire', async () => {
  const { router, ids, store } = await rig();
  const d = (await GET(router, `/api/v1/skills/${ids[0]}`)).body.data;
...
test('TC-149 DEC-018 rights.state travels on the wire as an explicit value', async () => {
  const { router, ids, store } = await rig();
  const known = (await GET(router, `/api/v1/skills/${ids[0]}`)).body.data;   // MIT
  const unknown = (await GET(router, `/api/v1/skills/${ids[1]}`)).body.data; // no licence
...
test('TC-150 REQ-062/DEC-009 content is NEVER served, under any licence', async () => {
  const { router, ids, store } = await rig();
  for (const id of ids) {
    const d = (await GET(router, `/api/v1/skills/${id}`)).body.data;
...
test('TC-151 REQ-061 every response carries the notice and attribution', async () => {
  const { router, ids, store } = await rig();
  const one = (await GET(router, `/api/v1/skills/${ids[0]}`)).body;
  const many = (await GET(router, '/api/v1/skills')).body;
...
test('TC-152 REQ-061 the serialiser REFUSES a record without attribution', async () => {
  ...
  const { store } = await rig({ skills: 1 });
  const row = (await store.cursorScan()).rows[0];
...
test('TC-162 REQ-061 a corrupt stored record produces a 500, never a silent omission', async () => {
  const { store, ids } = await rig({ skills: 1 });
  // Bypass every domain guard to simulate a record that got in some other way.
  const broken = { ...(await store.cursorScan()).rows[0], attribution_url: '' };
  const router = new ApiRouter({
    store: { schemaVersion: async () => 1, getCanonical: async () => broken,
             cursorScan: async () => ({ rows: [], cursor: { next: null, limit: 50 } }),
             listOccurrences: async () => ({ rows: [], cursor: { next: null, limit: 50 } }) },
    clock, limiter: null });
  const r = await router.handle({ method: 'GET', path: `/api/v1/skills/${ids[0]}`, query: {} });
...
test('TC-153 NFR-039 collections are cursor-paginated with an enforced maximum', async () => {
  const { router, store } = await rig({ skills: 25 });
  const seen = new Set();
  let cursor = null, pages = 0;
  do {
    const r = await GET(router, '/api/v1/skills', { cursor, limit: 7 });
  ...
  const capped = await GET(router, '/api/v1/skills', { limit: 9999 });
  assert.equal(capped.body.cursor.limit, 100, 'page size is capped, not honoured blindly');
  store.close();
});

test('TC-154 REQ-067 occurrences are paginated and 404 for an unknown skill', async () => {
  const { router, ids, store } = await rig();
  const ok = await GET(router, `/api/v1/skills/${ids[0]}/occurrences`);
  assert.equal(ok.status, 200);
  assert.ok(Array.isArray(ok.body.data));
  assert.ok('cursor' in ok.body);
  const missing = await GET(router, '/api/v1/skills/cs_nope/occurrences');
  assert.equal(missing.status, 404);
  assert.equal(missing.body.error.code, 'SKILL_NOT_FOUND');
  store.close();
});

test('TC-155 REQ-068 a source is returned with its declared access policy', async () => {
  const { router, store } = await rig();
  const r = await GET(router, '/api/v1/sources/gitskills');
  assert.equal(r.status, 200);
  assert.equal(r.body.data.access_policy.max_concurrency, 1);
  assert.equal((await GET(router, '/api/v1/sources/nope')).status, 404);
  store.close();
});

test('TC-156 REQ-069 search requires a query and returns matches', async () => {
  const { router, store } = await rig({ skills: 5 });
  assert.equal((await GET(router, '/api/v1/search')).status, 400);
  assert.equal((await GET(router, '/api/v1/search')).body.error.code, 'MISSING_QUERY');
  const r = await GET(router, '/api/v1/search', { q: 'skill-3' });
  assert.equal(r.status, 200);
...
test('TC-157 INVALID_CURSOR is a 400 with a recovery path, not a 500', async () => {
  const { router, store } = await rig();
  const r = await GET(router, '/api/v1/skills', { cursor: 'garbage' });
...
test('TC-158 REQ-097 rate limiting returns 429 with Retry-After', async () => {
  let t = 0;
  const limiter = new MemoryRateLimiter({ budget: 3, windowMs: 60_000, clock: () => t });
  const { router, store } = await rig({ limiter });
  for (let i = 0; i < 3; i++) assert.equal((await GET(router, '/api/v1/health')).status, 200);
  const blocked = await GET(router, '/api/v1/health');
  assert.equal(blocked.status, 429);
  assert.equal(blocked.body.error.code, 'RATE_LIMITED');
  assert.ok(Number(blocked.headers['Retry-After']) > 0, 'Retry-After is stated, as we require of sources');
  t += 60_001;
  assert.equal((await GET(router, '/api/v1/health')).status, 200, 'the window rolls');
  store.close();
});

test('TC-159 REQ-097 rate limits are per client, not global', async () => {
  const limiter = new MemoryRateLimiter({ budget: 2, windowMs: 60_000, clock: () => 0 });
  const { router, store } = await rig({ limiter });
  await GET(router, '/api/v1/health', {}, 'alice');
  await GET(router, '/api/v1/health', {}, 'alice');
  assert.equal((await GET(router, '/api/v1/health', {}, 'alice')).status, 429);
  assert.equal((await GET(router, '/api/v1/health', {}, 'bob')).status, 200, 'bob is unaffected');
  store.close();
});

test('TC-160 the API is read-only: writes are refused', async () => {
  const { router, store } = await rig();
  const r = await router.handle({ method: 'POST', path: '/api/v1/skills', query: {} });
  assert.equal(r.status, 405);
  assert.match(r.body.error.message, /read-only/);
  store.close();
});

test('TC-161 REQ-071 errors carry a stable code and a request id, and leak nothing', async () => {
  const { router, store } = await rig();
  const r = await GET(router, '/api/v1/nonsense');
...
test('TC-238 REQ-033 the API has no route to raw content and never returns raw bytes', async () => {
  const { router, ids, store } = await rig({ skills: 2 });
  // No raw route exists.
  for (const path of ['/api/v1/raw', `/api/v1/skills/${ids[0]}/raw`, '/api/v1/objects',
                      `/api/v1/raw/${ids[0]}`, '/api/v1/content']) {
    const r = await GET(router, path);
  ...
  for (const path of [`/api/v1/skills/${ids[0]}`, '/api/v1/skills', '/api/v1/search?q=skill']) {
    const body = JSON.stringify((await GET(router, path.split('?')[0], { q: 'skill' })).body);
